...
const getTasks = async (req, res) => {
    ...
    const user_id = (
        await connection.query('SELECT ID FROM users where username=$1', [user])
    ).rows[0].id
    ...
    const tasks = (
        await connection.query(
            'SELECT * FROM ordering INNER JOIN tasks ON tasks.ID = ordering.task_id WHERE ordering.user_id=$1 ORDER BY urgency, ind',
            [user_id]
        )
    ).rows
    // console.log(tasks);
