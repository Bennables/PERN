...
const OrgFind = () => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      setLoading(true)
      const res = await axios.get(`${link}/org/find`, { params: { name: trimmed } })
      ...
      if (e?.response?.status === 404) {
        ...
        setError(e?.response?.data?.message ?? 'Failed to find organization')
      }
    } finally {
      setLoading(false)
    }
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
          <div className="text-center mb-8">
            <div className="mx-auto h-12 w-12 rounded-full flex items-center justify-center mb-4" style={{ backgroundColor: '#cefad0' }}>
              <svg className="h-6 w-6" style={{ color: '#3a8c1a' }} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6V4m0 2a2 2 0 100 4m0-4a2 2 0 110 4m-6 8a2 2 0 100-4m0 4a2 2 0 110-4m12 0a2 2 0 100-4m0 4a2 2 0 110-4m-6 6v-2m0 2a2 2 0 100-4m0 4a2 2 0 110-4" />
              </svg>
            </div>
          ...
          <div className="mt-6 text-center space-y-2">
            <p className="text-sm text-gray-600">
              Need an account?{' '}
              <Link to="/register" className="font-medium transition-colors duration-200" style={{ color: '#3a9c10' }} onMouseEnter={e => (e.currentTarget.style.color = '#2d7a0c')} onMouseLeave={e => (e.currentTarget.style.color = '#3a9c10')}>
                Create one
              </Link>
            </p>
            <p className="text-sm text-gray-600">
              Don't have an org yet?{' '}
              <Link to="/org/create" className="font-medium transition-colors duration-200" style={{ color: '#3a9c10' }} onMouseEnter={e => (e.currentTarget.style.color = '#2d7a0c')} onMouseLeave={e => (e.currentTarget.style.color = '#3a9c10')}>
                Create org
              </Link>
            </p>
            <p className="text-xs text-gray-500">You can also continue without an org, but team features won't work.</p>
            <button
              type="button"
              onClick={() => {
