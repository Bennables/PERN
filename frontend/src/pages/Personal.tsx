  ...
  useEffect(() => {
    const checkOrg = async () => {
      try {
        ...
        if (e?.response?.status === 400 && msg === 'User is not part of any organization') {
          setHasOrg(false)
        } else if (e?.response?.status === 401 || msg === 'token expired') {
      ...
      <div className="bg-white border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-slate-600">Personal + Organization (if you have one)</p>
          </div>
